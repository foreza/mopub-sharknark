...
- (void)loadAdWithTargeting:(MPAdTargeting *)targeting
{
    ...
    NSLog(@"MPBannerAdManager, loadAdWithTargeting adLoadAttempt");

    
    if (!self.hasRequestedAtLeastOneAd) {
        self.hasRequestedAtLeastOneAd = YES;
        NSLog(@"MPBannerAdManager, hasRequestedAtLeastOneAd");
    }
    
    if (self.loading) {
        MPLogEvent([MPLogEvent error:NSError.adAlreadyLoading message:nil]);
        NSLog(@"MPBannerAdManager, self.loading");
        return;
    }

    self.targeting = targeting;
    
    NSLog(@"MPBannerAdManager, loadAdWithURL nil");
...
- (void)loadAdWithURL:(NSURL *)URL
{
    ...
    NSLog(@"MPBannerAdManager, loadAdWithURL URL.absoluteString loadURL: %@", URL.absoluteString);
    ...
    NSLog(@"MPBannerAdManager, requestingAdapter: nil");
    ...
    NSLog(@"MPBannerAdManager, self.communicator loadURL: %@", URL.absoluteString);
...
- (void)adapter:(MPBaseBannerAdapter *)adapter didFinishLoadingAd:(UIView *)ad
{
    ...
    NSLog(@"MPBannerAdManager, adapter: didFinishLoadingAd");
    
    if (self.requestingAdapter == adapter) {
        self.remainingConfigurations = nil;
        self.requestingAdapterAdContentView = ad;
        
        NSLog(@"MPBannerAdManager, self.requestingAdapter == adapter");
...
- (void)adapter:(MPBaseBannerAdapter *)adapter didFailToLoadAdWithError:(NSError *)error
{
    ...
    NSLog(@"MPBannerAdManager, adapter didFailToLoadAdWithError");
    ...
    if (self.requestingAdapter == adapter) {
        ...
        if (self.remainingConfigurations.count > 0) {
            ...
            NSLog(@"MPBannerAdManager, requestingAdapter == adapter, fetchAdWithConfiguration");
        ...
        // No more configurations to try. Send new request to Ads server to get more Ads.
            ...
            NSLog(@"MPBannerAdManager, requestingConfiguration nextURL != nil: %@", self.requestingConfiguration.nextURL.absoluteString);
        ...
        // No more configurations to try and no more pages to load.
            ...
            NSLog(@"MPBannerAdManager, clearResponseError clearResponseError");

            
        }
    }
    
        // we know the adapter is not nil here, and that the onscreenadapter == adapter
    

    if (self.onscreenAdapter == adapter && adapter != nil) {
        
        NSLog(@"MPBannerAdManager, onscreenAdapter has failed");
        ...
        if (self.adActionInProgress) {
            ...
            NSLog(@"MPBannerAdManager, adActionInProgress ");

        }
        if (self.requestingAdapterIsReadyToBePresented) {
            [self presentRequestingAdapter];
            NSLog(@"MPBannerAdManager, presentRequestingAdapter");

        } else {
            [self loadAdWithTargeting:self.targeting];
            // [self loadAdWithURL:self.requestingConfiguration.nextURL];       // this will stop the waterfall, but at least stop this destructive step
            // self.requestingConfiguration = [self.remainingConfigurations removeFirst];
               // [self fetchAdWithConfiguration:self.requestingConfiguration];

            NSLog(@"MPBannerAdManager, loadAdWithTargeting"); // THIS GETS CALLED WHY
